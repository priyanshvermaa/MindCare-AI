...
import { Brain, Lock, Mail, Eye, EyeOff, Sparkles, Smile, BookOpen, ShieldCheck } from 'lucide-react';
...
export default function Login() {
  ...
  const { user, loading, loginUser, loginWithToken } = useAuth();
  ...
  // Handle URL redirect query tokens or errors
  useEffect(() => {
    ...
    if (tokenParam) {
      ...
      loginWithToken(tokenParam)
        ...
        .catch((err) => {
          setLocalError(err.message || 'Verification of OAuth session token failed.');
        })
        .finally(() => {
          setSubmitting(false);
        });
  ...
  }, [location.search, navigate, redirectPath, loginWithToken]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Sidebar Permission:', data.user?.role === 'admin' ? 'GRANTED' : 'DENIED');
        console.log('------------------------');
      } catch (dbgErr) {
        console.warn('Failed to parse debug token:', dbgErr.message);
      }
  ...
  // Real OAuth Login triggers redirecting to Backend Router endpoints
  ...
  return (
    <div className="min-h-screen bg-[#FAF8FF] flex items-center justify-center p-4 md:p-6 lg:p-8 font-poppins text-[#111827]">
      <div className="w-full max-w-6xl bg-white rounded-[32px] overflow-hidden shadow-xl shadow-purple-100/25 border border-[#E5E7EB] grid grid-cols-1 lg:grid-cols-11 min-h-[640px] md:min-h-[700px]">
        ...
        {/* Right Section (≈55%) */}
        <div className="lg:col-span-6 p-8 md:p-12 lg:p-16 flex flex-col justify-center items-center">
          ...
          <div className="w-full max-w-lg bg-white rounded-[30px] shadow-xl shadow-purple-100/10 border border-[#E5E7EB] p-8 md:p-12 text-left relative z-10">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              {/* Email field (height 56px, i.e., h-14) */}
              <div>
              ...
              {/* Password field (height 56px, i.e., h-14) */}
              <div>
              ...
              <div className="flex items-center justify-between pt-1 select-none">
                <label className="flex items-center gap-2 text-xs font-bold text-[#6B7280] cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 rounded text-[#7C4DFF] focus:ring-[#7C4DFF] border-[#E5E7EB]"
                  />
                  Remember Me
                </label>
                <Link
                  to="/forgot-password"
                  className="text-xs font-bold text-[#7C4DFF] hover:text-[#6D3FEA] transition-colors"
                >
                  Forgot Password?
                </Link>
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-gradient-to-r from-[#7C4DFF] to-[#9F7AEA] hover:from-[#6D3FEA] hover:to-[#8E69DB] text-white font-bold text-sm py-4 rounded-full shadow-md shadow-[#7C4DFF]/15 hover:shadow-lg hover:shadow-[#7C4DFF]/25 transition-all hover:scale-[1.02] active:scale-[0.98] mt-6 disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-1.5"
              >
                {submitting ? 'Authenticating...' : 'Get Started'}
            ...
            <div className="mt-5 grid grid-cols-2 gap-4">
              <button
                ...
                disabled={submitting}
                className="flex items-center justify-center gap-2 py-3 px-4 border border-[#E5E7EB] rounded-full text-xs font-bold text-gray-700 bg-white hover:bg-gray-50/50 shadow-sm transition-all active:scale-95 disabled:opacity-50"
              >
            ...
            <div className="mt-8 text-center text-xs font-semibold text-gray-500">
              ...
              <Link to="/register" className="font-bold text-[#7C4DFF] hover:text-[#9F7AEA] transition-colors ml-1">
                Create Account
              </Link>
            </div>
